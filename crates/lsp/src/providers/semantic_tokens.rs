...
fn collect_posting(content: &Rope, posting: &ast::Posting<'_>, out: &mut Vec<RawToken>) {
...
fn collect_cost_spec(content: &Rope, cost_spec: &ast::CostSpec<'_>, out: &mut Vec<RawToken>) {
...
fn collect_number_expr(content: &Rope, expr: &ast::NumberExpr<'_>, out: &mut Vec<RawToken>) {
    match expr {
...
fn collect_tolerance(content: &Rope, tolerance: &ast::WithSpan<&str>, out: &mut Vec<RawToken>) {
...
fn push_span(content: &Rope, span: ast::Span, kind: TokenKind, out: &mut Vec<RawToken>) {
