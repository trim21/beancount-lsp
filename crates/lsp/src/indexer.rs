  ...
  pub fn update_document(&mut self, uri: Url, text: String) -> bool {
    ...
    if let Some(target_key) = &target_key {
      let dupes: Vec<Url> = self
        ...
        .keys()
        .filter(|existing| {
          Self::url_normalized_key(existing).as_ref() == Some(target_key)
        })
        .cloned()
        .collect();
      for k in dupes {
...
mod tests {
  ...
  fn ref_counts_follow_include_graph() {
    ...
    let root_uri =
      Url::from_file_path(canonical_or_original(&root_path)).expect("root uri");
    ...
    let updated =
      indexer.update_document(root_uri.clone(), "include \"a.bean\"\n".to_owned());
    assert!(updated);
